...
`
	...
	t.Setenv("SYNCCAL_WEB_TOKEN", "from-env")
	t.Setenv("SYNCCAL_SYNC_INTERVAL", "30m")
	t.Setenv("SYNCCAL_SYNC_FILTER_PRIVATE", "true")
	t.Setenv("SYNCCAL_DESTINATIONS_0_PASSWORD", "from-env-token")
	t.Setenv("SYNCCAL_DATABASE_PATH", "/data/synccal.db")

	cfg, err := Load(path)
	require.NoError(t, err)
	assert.Equal(t, "from-env", cfg.Web.Token)
	assert.Equal(t, "30m", cfg.Sync.Interval)
	assert.True(t, cfg.Sync.FilterPrivate)
	assert.Equal(t, "from-env-token", cfg.Destinations[0].Password)
	assert.Equal(t, "/data/synccal.db", cfg.Database.Path)
	// Non surchargé : valeur du fichier conservée
	assert.Equal(t, "https://example.com/cal.ics", cfg.Sources[0].URL)
}
...
`
	path := writeTempYAML(t, yaml)
	// Variable définie mais vide : ne doit PAS écraser la valeur du fichier
	t.Setenv("SYNCCAL_WEB_TOKEN", "")

	cfg, err := Load(path)
	require.NoError(t, err)
	assert.Equal(t, "keep-me", cfg.Web.Token)
}
...
`
	...
	_, err := Load(path)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "SYNCCAL_SYNC_BATCH_SIZE")
}
...
`
	...
	// Le chemin de fichier n'existe pas : SYNCCAL_CONFIG est prioritaire
